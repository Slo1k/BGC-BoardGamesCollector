...
import com.example.bgc.databinding.ActivityListBinding
...
class GamesListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val options = arrayOf("tytuł", "data wydania", "ranking")
        binding.ssGames.adapter = ArrayAdapter<String>(
            this,
            android.R.layout.simple_list_item_1,
            options
        )

        binding.ssGames.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
    ...
    private fun displayData() {
        val data = dbHandler.readBoardGames(sortColumn)
        binding.tlGames.removeAllViews()
        ...
        for (i in 0 until data.size) {
            ...
            binding.tlGames.gravity = Gravity.CENTER
            ...
            when {
                ...
                data[i].description?.length!! > 200 -> descriptionView.text =
                    data[i].description?.substring(0, 200) + "..."
                else -> descriptionView.text = data[i].description
            }
            Picasso.get().load(data[i].image).resize(200, 0).into(imageView)

            binding.tlGames.addView(tableRow)
    ...
    private fun rankInfo(game: Game){
        intent = Intent(this, RankingActivity::class.java)
        intent.putExtra("Game", game.title.toString())
